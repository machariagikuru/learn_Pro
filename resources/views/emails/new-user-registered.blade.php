...
<body>
    ...
    <img src="https://drive.google.com/uc?export=view&id=1Dp5bAnPuKV882NyfNZAmJ0ECgSIbI5cE" alt="Learn Pro Logo" class="logo">
        <div class="greeting">New User Registration Alert!</div>
        <div class="intro">
            A new user has registered on your platform.
        </div>
        <div class="user-info">{{ $newUser->first_name }} {{ $newUser->last_name }}</div>
        <div class="features">
            <ul>
                <li>Email: {{ $newUser->email }}</li>
                <li>Registration Date: {{ $newUser->created_at->format('M d, Y H:i') }}</li>
                <li>User ID: {{ $newUser->id }}</li>
            </ul>
        </div>
        <a href="{{ url('/admin/users') }}" class="button">View User Details</a>
        <div class="footer">
